...
// Initialization ($(window).on("load", ...)):
// The code waits for the window to load before creating an instance of the Atlas class and calling its init method.
...
class Atlas {
    ...
    this.solutionsDataIndex = [
      {
        ...
        items: [
          { field: "DSR%", color: "darkgreen", name: "DSR" },
          {
            field: "PrWtrMng",
            color: "darkgreen",
            name: "Precision water management",
          },
          {
            field: "DroughtTlSeed%",
            color: "darkgreen",
            name: "Drought tolerant seeds",
          },
  ...
  //   Atlas Class:
  // Constructor (constructor()): Initializes several properties related to map configuration, data sources, and UI elements. These include settings for the map, layers, filters, and data sources.
  // Initialization (init): Sets up the map by removing any existing map instance, creating a new map, adding a tile layer, and loading initial data.
  // Load Data (loadData): Loads data from an Excel file, fills filter options for countries, commodities, risks, solutions, and analytics, and sets default values based on cookies.
  // Enable Screenshot (enableScreenshot): Adds functionality to take a screenshot of the map when a button is clicked, using the domtoimage library.
  // Fill Commodity Options (fillCommodityOptions): Populates the commodity filter options in the UI based on data loaded from the Excel file.
  // Enable Commodity Options (enableCommodityOptions): Toggles the visibility of commodity items based on the state of checkboxes.
  ...
  init = () => {
    ...
    this.streetLayer = L.tileLayer(this.streetLayerTilesUrl, {
      attribution: null,
      zoomSnap: 0.1,
      opacity: 1 / 3,
      transparency: "true",
  ...
  loadData = () => {
    this.loadExcelFile(`data/${this.lookupSheet}`)
      ...
      .then(() => {
        ...
        this.filterCountry
          .val(this.cookieObject.countryId || 0)
          .trigger("change");
        $(
          `input[name="radio-commodity"][value="${
            this.cookieObject.commodityId || 1
          }"]`
        )
          .prop("checked", true)
          .trigger("change");
        let commodityParentCheckboxId = $(
          `input[name="radio-commodity"]:checked`
        ).data("parent-id");
        $(
          `input[name="checkbox-commodity-group"][data-id="${commodityParentCheckboxId}"]`
        )
          .prop("checked", true)
          .trigger("change");
        $(
          `input[name="checkbox-commodity-group"]:not([data-id="${commodityParentCheckboxId}"])`
        )
          .prop("checked", false)
          .trigger("change");
      })
  ...
  //   Map Configuration: Uses Leaflet to initialize and configure a map with a specified tile layer for street maps.
  //   Excel Data Handling: The loadData method processes data from an Excel file to populate various filters and lists.
  //   UI Interactions: Includes functionality for updating UI elements (like commodity checkboxes) based on user interactions and data.

  // COMMODITIES
  fillCommodityOptions = () => {
    let groups = this.uqArray(this.commodityList.map((a) => a.group));
    let commodityFilterHtml = groups
      .map((a, i) => {
        let commodityItemsHtml = this.commodityList
          .filter((b) => b.group == a)
          .map((b) => {
            let optionDisability = b.status ? "" : "disabled";
            let optionDataAttrs = b.status
              ? `data-workspace="${b.workspace}" data-layer="${b.layer}" data-status="${b.status}" data-label="${b.label}"`
              : "";
  ...
  //   fillCommodityOptions Method
  //   Purpose: Populates the commodity filter options in the UI.
  //   Process:
  //   Groups Commodities: Uses uqArray to get unique groups from commodityList.
  //   Generates HTML: Creates HTML for commodity filters, grouping them by their group attribute.
  //   Updates UI: Inserts the generated HTML into filterCommodity.

  enableCommodityOptions = () => {
    $("input[type='checkbox'][name='checkbox-commodity-group']").on(
      "change",
      (e) => {
        let currentGroup = $(e.currentTarget);
        let commodityGroupItems = $(
          `div#commodity-list-${currentGroup.data("id")}`
        );
        currentGroup.prop("checked")
          ? commodityGroupItems.show()
          : commodityGroupItems.hide();
      }
    );
    $("input[type='checkbox'][name='checkbox-commodity-group']:first")
      .prop("checked", true)
      .trigger("change");
    $("input[type='radio'][name='radio-commodity']").on("change", (e) => {
      ...
      this.chosenCommodity = this.commodityList.find(
        (b) => b.id == this.chosenCommodityId
      ).label;
      this.commodityLabel.empty().html(`<b>${this.chosenCommodity}</b>`);
      // filter the other layers based on selected commodity
      this.riskList = this.fullRiskList.filter(
        (a) => a.commodity_id == this.chosenCommodityId
      );
      this.solutionList = this.fullSolutionList.filter(
        (a) => a.commodity_id == this.chosenCommodityId
      );
      this.analyticsList = this.fullAnalyticsList.filter(
        (a) => a.commodity_id == this.chosenCommodityId
      );
      this.plotRasterLayer(
        currentItem.data("workspace"),
        currentItem.data("layer")
      );
      this.currentRasterType = "commodity";
    ...
    $("input[type='radio'][name='radio-commodity']:first")
      .prop("checked", true)
      .trigger("change");
  };
  //   enableCommodityOptions Method
  //   Purpose: Adds event handlers to handle interactions with commodity options.
  //   Process:
  //   Checkbox Interaction: Toggles visibility of commodity options based on checkbox states.
  //   Radio Button Interaction: Updates selected commodity and associated data when a radio button is changed.
  //   Updates UI: Sets chosenCommodityId and updates related labels and data filters. Calls methods to update risks, solutions, and analytics based on the selected commodity.

  loadCommoditySolutions = () => {
    this.chosenSolutionsIndexItem = this.solutionsDataIndex.find(
      (a) => a.commodity_id == this.chosenCommodityId
    );
    if (this.chosenSolutionsIndexItem) {
      ...
      this.loadCSVData(`data/${csvFile}`)
        .then(
          (solutionsCSVData) =>
            (this.currentSolutionData = this.csvToArray(solutionsCSVData))
        )
        .catch((err) => {
  ...
  //   loadCommoditySolutions Method
  //   Purpose: Loads solution data related to the selected commodity.
  //   Process:
  //   Finds Solutions: Retrieves the CSV file name for solutions based on chosenCommodityId.
  //   Loads Data: Loads and processes CSV data into currentSolutionData.

  // RISKS
  fillRiskOptions = () => {
    if (this.riskList.length) {
      let types = this.uqArray(this.riskList.map((a) => a.type));
      let riskFilterHtml = types
        .map((a, i) => {
          let group1Items = this.uqArray(
            this.riskList.filter((b) => b.type == a).map((b) => b.group1)
          );
          let group1ItemsHtml = group1Items
            .map((b, j) => {
              // ungrouped risks
              let ungroup2Items = this.riskList.filter(
                (c) => c.group1 == b && c.type == a && !c.group2
              );
              let ungroup2ItemsHtml = "";
              if (ungroup2Items.length) {
                ungroup2ItemsHtml = ungroup2Items
                  .map((c) => {
                    let optionDisability = c.status ? "" : "disabled";
                    let optionDataAttrs = c.status
                      ? `data-workspace="${c.workspace}" data-layer="${c.layer}" data-status="${c.status}" data-label="${c.label}"`
                      : "";
              ...
              // grouped risks
              let group2Items = this.riskList.filter(
                (c) => c.group1 == b && c.type == a && c.group2
              );
              let group2ItemsHtml = "";
              if (group2Items.length) {
                let group2s = this.uqArray(group2Items.map((c) => c.group2));
                group2ItemsHtml = group2s
                  .map((c, k) => {
                    let items = group2Items.filter((d) => d.group2 == c);
                    let risksUnderGroup2Html = items
                      .map((d) => {
                        let optionDisability = d.status ? "" : "disabled";
                        let optionDataAttrs = d.status
                          ? `data-workspace="${d.workspace}" data-layer="${d.layer}" data-status="${d.status}" data-label="${d.label}"`
                          : "";
              ...
              return `<div class="col-sm-12 col-md-6 col-lg-6 form-check form-switch mt-1 ms-1">
                        ...
                        <div id="risk-list-${j}-${i}" style="display: none;">${
                ungroup2ItemsHtml + group2ItemsHtml
              }</div>
                    </div>`;
            })
            .join("\n");
  ...
  // fillRiskOptions Method
  // Purpose: Populates the risk filter options in the UI.
  // Process:
  // Groups Risks: Groups risks by type and sub-groups.
  // Generates HTML: Creates HTML for risk filters, including checkboxes and radio buttons.
  // Updates UI: Inserts generated HTML into filterRisk.
  ...
  enableRiskOptions = () => {
    ...
    // Group 1
    $("input[type='checkbox'][name='checkbox-risk-group1']").on(
      "click",
      (e) => {
        let currentGroup = $(e.currentTarget);
        let riskGroup1Items = $(`div#risk-list-${currentGroup.data("id")}`);
        currentGroup.prop("checked")
          ? riskGroup1Items.show()
          : riskGroup1Items.hide();
      }
    );
    // Group 2
    $("input[type='checkbox'][name='checkbox-risk-group2']").on(
      "click",
      (e) => {
        let currentGroup = $(e.currentTarget);
        let riskGroup2Items = $(`div#risk-list-${currentGroup.data("id")}`);
        currentGroup.prop("checked")
          ? riskGroup2Items.show()
          : riskGroup2Items.hide();
      }
    );
    // Main layers
    $("input[type='radio'][name='radio-risk']").on("change", (e) => {
      ...
      this.chosenRisk = this.riskList.find(
        (b) => b.id == this.chosenRiskId
      ).label;
      let chosenRiskType = this.riskList.find(
        (b) => b.id == this.chosenRiskId
      ).type;
      this.riskLabel
        .empty()
        .html(`<b>${this.chosenRisk} (${chosenRiskType})</b>`);
      this.plotRasterLayer(
        currentItem.data("workspace"),
        currentItem.data("layer")
      );
      ...
      $("input[type='radio'][name='radio-solution']:checked").prop(
        "checked",
        false
      );
      this.solutionLabel.empty();
      $("input[type='radio'][name='radio-analytics']:checked").prop(
        "checked",
        false
      );
      this.analyticsLabel.empty();
  ...
  //   enableRiskOptions Method
  //   Purpose: Adds event handlers to handle interactions with risk options.
  //   Process:
  //   Checkbox Interaction: Toggles visibility of risk options based on checkbox states.
  //   Radio Button Interaction: Updates selected risk and related data, hides other options, and updates the UI accordingly.

  // SOLUTIONS
  fillSolutionOptions = () => {
    if (this.solutionList.length) {
      let groups = this.uqArray(this.solutionList.map((a) => a.group));
      let solutionFilterHtml = groups
        .map((a, i) => {
          let solutionItemsHtml = this.solutionList
            .filter((b) => b.group == a)
            .map((b) => {
              let optionDisability = b.status ? "" : "disabled";
              let optionDataAttrs = b.status
  ...
  // fillSolutionOptions Method
  // Purpose: Populates the solution filter options in the UI.
  // Process:
  // Groups Solutions: Groups solutions by their group attribute.
  // Generates HTML: Creates HTML for solution filters, similar to the commodity and risk options.
  // Updates UI: Inserts generated HTML into filterSolution.

  enableSolutionOptions = () => {
    // Group
    $("input[type='checkbox'][name='checkbox-solution-group']").on(
      "change",
      (e) => {
        let currentGroup = $(e.currentTarget);
        let solutionGroupItems = $(
          `div#solution-list-${currentGroup.data("id")}`
        );
        currentGroup.prop("checked")
          ? solutionGroupItems.show()
          : solutionGroupItems.hide();
      }
    );
    // Main Layer
    $("input[type='radio'][name='radio-solution']").on("change", (e) => {
      ...
      $("input[type='radio'][name='radio-risk']:checked").prop(
        "checked",
        false
      );
      this.riskLabel.empty();
      ...
      this.chosenSolution = this.solutionList.find(
        (b) => b.id == this.chosenSolutionId
      ).label;
      let analyticsDescription = this.solutionList.find(
        (b) => b.id == this.chosenSolutionId
      ).description;
  ...
  // enableSolutionOptions Method
  // Purpose: Adds event handlers to handle interactions with solution options.
  // Process:Checkbox Interaction: Toggles visibility of solution options based on checkbox states.
  // Radio Button Interaction: Updates selected solution and related data, and clears other options.
      ...
      let analyticsFilterHtml =
        ...
        this.analyticsList
          .map((a) => {
            ...
            let optionDataAttrs = a.status
              ? `data-workspace="${a.workspace}" data-layer="${a.layer}" data-status="${a.status}" data-label="${a.label}"`
              : "";
            return `<div class="col-sm-12 col-md-6 col-lg-6 form-check form-switch mt-0 ms-1">
  ...
  //   fillAnalyticsOptions Method
  //   Purpose: Populates the analytics options in the UI and sets up event handlers.
  //   Steps:
  //   Checks if there are available analytics options.
  //   Creates HTML for a reference shortcut and analytics options.
  //   Updates the filterAnalytics container with the generated HTML.
  //   Sets up event handlers for radio button changes.
  ...
  enableAnalyticsOptions = () => {
    $("input[type='radio'][name='radio-analytics']").on("change", (e) => {
      ...
      this.chosenAnalytics = this.analyticsList.find(
        (b) => b.id == this.chosenAnalyticsId
      ).label;
  ...
  // enableAnalyticsOptions Method
  // Purpose: Enables analytics options and handles user interactions.
  // Steps:
  // Sets up a change event handler for radio buttons.
  // Updates selected analytics, removes previous selections, and plots the selected raster layer.
  ...
  plotRasterLayer = (workspace, layer) => {
    ...
    if (this.rasterLayerLegend !== null)
      this.map.removeControl(this.rasterLayerLegend);
  ...
  //   Steps:
  //   Constructs URLs for the WMS service and legend.
  //   Removes any existing layers and adds the new one.
  //   Updates the map legend with the new layer’s legend.

  // LOCATIONS: Country:State > State:District
  fillCountryOptions = () => {
    let filterCountryHtml = this.countryList
      .filter((a) => a.status)
    ...
    this.filterCountry.unbind("change").on("change", () => {
      ...
      this.chosenCountryObj = this.countryList.find(
        (a) => a.id == this.chosenCountryId
      );
  ...
  //   fillCountryOptions Method
  //   Purpose: Populates the country dropdown in the UI.
  //   Steps:Creates HTML options for countries and updates the filterCountry dropdown.
  //   Sets up a change event handler to handle country selection changes.
  ...
  fillStateOptions = () => {
    if (this.chosenCountryId) {
      ...
      let filterStateHtml = this.statesGeoData.features
        .map((a) => a.properties)
        .map((a) => `<option value="${a.S_CODE}">${a.S_NAME}</option>`)
        .join("\n");
  ...
  //   fillStateOptions Method
  //   Purpose: Populates the state dropdown based on the selected country.
  //   Steps:Creates HTML options for states if a country is selected; otherwise, shows a default empty option.
  ...
  enableStateChange = () => {
    if (this.chosenCountryId)
      this.filterState
        .unbind("change")
        .on("change", () =>
          this.onStateChange(parseInt(this.filterState.val()))
        );
    else this.filterState.unbind("change");
  };
  //   enableStateChange Method
  //   Purpose: Sets up an event handler for state selection changes.
  //   Steps:
  //   If a country is selected, binds a change to the state dropdown.

  onCountryChange = () => {
    this.locationLabel.empty().html(`<b>${this.chosenCountryObj.label}</b>`);
    let [countriesMapFile, statesMapFile] = [
      this.chosenCountryObj.outline_map,
      this.chosenCountryObj.states_map,
    ];
    this.chosenState = null;
    this.chosenStateId = null;
    if (!this.chosenCountryId) {
      this.loadVectorFile(countriesMapFile)
        .then(
          (topoData) =>
            (this.countriesGeoData = topojson.feature(
              topoData,
        ...
        .catch((err) => {
          alert("Unable to load South Asia's country level boundaries");
          console.error(err);
        })
        .finally(() => this.stopWaiting());
    } else {
      this.loadVectorFile(statesMapFile)
        .then((topoData) => {
          this.statesGeoData = topojson.feature(
            topoData,
            topoData.objects.collection
          );
        })
        ...
        .catch((err) => {
          alert(
            `Unable to load ${this.chosenCountryObj.label}'s state level boundaries`
          );
          console.error(err);
        })
        .finally(() => this.stopWaiting());
  ...
  //   onCountryChange Method
  //   Purpose: Handles changes when a country is selected.
  //   Steps:
  //   Updates the location label and loads vector data for either countries or states based on the selection.
  //   Updates the map and state options accordingly.

  onStateChange = (chosenStateId) => {
    this.chosenStateId = chosenStateId;
    if (this.chosenStateId) {
      this.chosenState = this.statesGeoData.features
        .map((a) => a.properties)
        .find((a) => a.S_CODE == chosenStateId)?.S_NAME;
      this.locationLabel
        .empty()
        .html(`<b>${this.chosenState}, ${this.chosenCountryObj.label}</b>`);
      this.loadVectorFile(this.chosenCountryObj.districts_map)
        .then((topoData) => {
          this.districtsGeoData = topojson.feature(
            topoData,
            topoData.objects.collection
          );
          this.districtsGeoData.features =
            this.districtsGeoData.features.filter(
              (a) => a.properties.S_CODE == this.chosenStateId
            );
        })
        .then(() => {
          this.plotDistricts(chosenStateId);
          this.fitGeojsonData = L.geoJson(
            this.districtsGeoData,
            {}
          ).getBounds();
          this.map.fitBounds(this.fitGeojsonData);
        })
        .catch((err) => {
          alert(
            `Unable to load ${this.chosenCountryObj.label}'s district level boundaries`
          );
          console.error(err);
        })
        .finally(() => this.stopWaiting());
  ...
  //   Purpose: Handles changes when a state is selected.
  //   Steps:
  //   Updates the location label and loads vector data for districts within the selected state.
  //   Updates the map with district boundaries.

  removeSvgAll = () => {
    this.map.eachLayer((l) => {
      if (l.options.id && l.options.id.indexOf(`countries`) === 0)
        this.map.removeLayer(l);
      if (l.options.id && l.options.id.indexOf(`states`) === 0)
        this.map.removeLayer(l);
      if (l.options.id && l.options.id.indexOf(`districts`) === 0)
        this.map.removeLayer(l);
    });
  };

  // removeSvgAll Method
  // Purpose: Removes all SVG layers from the map.
  // Steps:
  // Iterates over all layers and removes those with specific IDs.
    ...
    let polygons = L.d3SvgOverlay(
      (selection, projection) => {
        ...
        locationGroup
          ...
          .attr(
            "style",
            "z-index:10000;pointer-events:visiblePainted !important"
          )
          .attr("stroke", "black")
          ...
          .on("mouseover", (e, d) => {
            d3.select(`country_${d.properties.C2_CODE}`).attr(
              "cursor",
              "pointer"
            );
    ...
    let polygons = L.d3SvgOverlay(
      (selection, projection) => {
        ...
        locationGroup
          ...
          .attr("fill-opacity", 0.25)
          .on("mouseover", (e, d) => {
            d3.select(`district_${d.properties.CN_DT_Code}`).attr(
              "cursor",
              "pointer"
            );
        ...
        locationGroup
          .transition()
          .duration(10)
          .attr("stroke-width", `${1 / pScale}px`);
      },
      { id: `districts` }
    );
    polygons.addTo(this.map);
  };
  //   plotCountries, plotStates, plotDistricts Methods
  //   Purpose: Renders boundaries for countries, states, or districts on the map.
  //   Steps:
  //   Uses L.d3SvgOverlay to add SVG overlays to the map.
  //   Sets attributes and event handlers for user interactions.

  showSolutionVisual = (e, d) => {
    debugger;
    let dataPoint = this.currentSolutionData.find(
      (a) => a.CN_DT_Code == d.properties.CN_DT_Code
    );
    let fieldsForChart = this.chosenSolutionsIndexItem.items.map(
      (a) => a.field
    );
    ...
    if (someValuesAvailable) {
      ...
      Highcharts.chart(this.popupGraphId, {
        ...
        yAxis: {
          title: { text: null, style: { color: "#000" } },
          labels: { style: { color: "#000" } },
          gridLineWidth: 0,
        },
  ...
  //   showSolutionVisual Method
  //   Purpose: Displays a chart showing the suitability of solutions for a selected district.
  //   Steps:
  //   Retrieves data for the selected district and generates a Highcharts column chart.
  //   Displays the chart or an error message if data is unavailable.

  loadVectorFile = (fileName) =>
  ...
  // loadVectorFile Method
  // Purpose: Loads vector data from a file.
  // Steps:Makes an AJAX request to fetch and parse vector map files.
  ...
  // excelToArray, loadExcelFile Methods
  // Purpose: Converts Excel files to arrays and loads them.
  // Steps:Reads Excel files and converts sheets to JSON arrays.
  ...
  // csvToArray, loadCSVData Methods
  // Purpose: Converts CSV text to arrays and loads CSV files.
  // Steps:Reads CSV data and converts it to JSON arrays.

  csvToArray = (csvText) => {
    let wb = XLSX.read(csvText, { type: "binary" });
    let rows = XLSX.utils.sheet_to_json(wb.Sheets.Sheet1, {
      header: 1,
      raw: false,
    });
    let header = rows[0];
    ...
    let data = body.map((a) => {
      ...
      header.forEach(
        (b, i) =>
          (result[b] = ["", undefined, null].includes(a[i])
            ? null
            : isNaN(a[i])
            ? a[i].replace(/\"/g, "")
            : this.num(a[i]))
  ...
  //   uqArray, sumArray, num Methods
  // Purpose: Utility functions for array operations and data conversion.
  // Steps:Provides functions for unique array values, summing arrays, and converting values to numbers.
...
// startWaiting, stopWaiting Methods
// Purpose: Controls loading indicators.
// Steps:Shows or hides a loading modal with a spinner.
