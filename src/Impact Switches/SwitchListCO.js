...
export default function SwitchesGroup({ activeImpact, changeImpact }) {
  const switchh = ["Rice", "Maize", "Wheat", "Sorghum", "Finger Millet", "Pearl Millet", "Chickpea", "Pigeon Pea", "Pulses", "Mustard", "Soybean"];
  const switchid = ["rice", "maize", "wheat", "sorghum", "fmillet", "pmillet", "chickpea", "ppea", "pulses", "mustard", "soybean"];
  ...
  return (
    <FormControl component="fieldset" variant="standard" sx={{ paddingBottom: 1, paddingLeft: 6 }}>
      {switchh.map((sname, index) => (
        <FormGroup>
          <FormControlLabel
            control={<AntSwitch inputProps={{ "aria-label": "ant design" }} checked={false} onChange={changeImpact(switchid[index])} name={switchid[index]} />}
            key={switchid[index]}
            label={
