...
export default function MApp({ activeCrop, focus = "Region", activeRegion, activeOpt, CurrRisk, activeImpact }) {
  ...
  const color1 = {
    color: [
      "palette",
      ["interpolate", ["linear"], ["/", ["-", nir, red], ["+", nir, red]], -0.1, 0, 3, 10],
      ["rgba(98, 181, 209, 0)", "#440154", "#3b528b", "#21918c", "#5ec962", "#fde725", "rgba(56, 150, 59, 1)", "rgba(98, 181, 209, 1)", "rgba(90, 230, 153, 1)", "rgba(98, 181, 209, 0)"],
    ],
  };
  ...
  useEffect(() => {
    ...
    if (activeCrop.rice) {
      if (CurrRisk["hindex"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_hindex.tif" }] });
      } else if (CurrRisk["DEMON"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_DEMON.tif" }] });
      } else if (CurrRisk["TEMLO"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_TEMLO.tif" }] });
      } else if (CurrRisk["TEMUP"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_TEMUP.tif" }] });
      }
      //else if(CurrRisk['COLLO']){source1 = new GeoTIFF({sources: [{ url: './Crop_TIFF_files/Rice_Hazard2/New_Rice_COLLO.tif' }],});}
      else if (CurrRisk["RAINF"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_RAINF.tif" }] });
      } else if (CurrRisk["HIRHI"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_HIRHI.tif" }] });
      } else if (CurrRisk["DRYSP"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_DRYSP.tif" }] });
      } else if (CurrRisk["DROMO"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_DROMO.tif" }] });
      }
      ...
      else if (CurrRisk["SPICO"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_SPICO.tif" }] });
      } else if (CurrRisk["SPIHE"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Crop_TIFF_files/Rice_Hazard2/New_Rice_SPIHE.tif" }] });
      }
      //else if(CurrRisk['SPIRA']){source1 = new GeoTIFF({sources: [{ url: './Crop_TIFF_files/Rice_Hazard2/New_Rice_SPIRA.tif' }],});}
      else if (activeOpt["ICT Agro Advisory"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_WEAGA.tif" }] });
      } else if (activeOpt["Levelling"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_LASLV.tif" }] });
      } else if (activeOpt["Zero Tillage"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_ZTILL.tif" }] });
      } else if (activeOpt["Broad Bed and Furrow"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_BBFIB.tif" }] });
      } else if (activeOpt["Early Sowing"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_ADPTI.tif" }] });
      } else if (activeOpt["DSR (Dry Seed)"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_DSDRY.tif" }] });
      } else if (activeOpt["DSR (Wet Seed)"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_DSWET.tif" }] });
      } else if (activeOpt["SRI"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_SRIUT.tif" }] });
      } else if (activeOpt["Crop insurance"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_INSUR.tif" }] });
      } else if (activeOpt["Stress tolerant variety"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_ADVAR.tif" }] });
      } else if (activeOpt["Deep Placement of Urea"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_DPURZ.tif" }] });
      }
      ...
      else if (activeOpt["Low Tech Precision Technology"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_PNMLT.tif" }] });
      } else if (activeOpt["High Tech Precision Technology"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_PNMHT.tif" }] });
      } else if (activeOpt["Microirrigation"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_MICIR.tif" }] });
      } else if (activeOpt["Precision Management"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_PWMGT.tif" }] });
      } else if (activeOpt["Farm Pond"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/EveryHazard2_Multiplicative_Suitability_Rice_WHSRC.tif" }] });
      } else if (activeImpact["Productivity"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/Rice_y.tif" }] });
      } else if (activeImpact["Value of Production"]) {
        source1 = new GeoTIFF({ sources: [{ url: "./Multiplicative_with_zeros/Rice_x.tif" }] });
      } else {
